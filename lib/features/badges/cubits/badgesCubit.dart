...
class BadgesCubit extends Cubit<BadgesState> {
  ...
  List<Badges> getRewards() {
    List<Badges> rewards = getAllBadges().where((element) => element.status != "0").toList();
    List<Badges> scratchedRewards = rewards.where((element) => element.status == "2").toList();
    List<Badges> unscratchedRewards = rewards.where((element) => element.status == "1").toList();
